/*  Handles calculation for current and monthly balances.
 *   Also creates, calculates data and updates the charts.
 */
class BalanceManager {
  ...
  //Is called when an item is new or changed
  //Recalculates all balance statistics and updates the charts/panels
  updateStatistics(widgetList) {
    ...
    for (let i = 0; i < widgetList.length; i++) {
      ...
      if (today.getMonth() + 1 === widgetList[i].date.split("-")[1] || "0" + (
          today.getMonth() + 1) === widgetList[i].date.split("-")[1]) {
  ...
  //Updates all charts and panels when the user is on the main screen
  updatePanels() {
    if (this.element !== null) {
      if (this.type === "main") {
        let chartSavingElement = this.element.querySelector("#savingChart"),
          chartElement = this.element.querySelector("#weekChart"),
          chartCategoryElement = this.element.querySelector("#categoryChart");

        //Creates and updates the Saving chart
        ...
        //Creates and updates the Week chart
        ...
        //Creates and updates the Category chart
        ...
        balanceElement.querySelector(".balance").innerHTML = this.balance +
          " €";
        balanceElement.querySelector(".balanceGain").innerHTML = "+ " + this
          .balancePositive + " €";
        balanceElement.querySelector(".balanceLoss").innerHTML = "- " + Math
          .abs(this.balanceNegative) + " €";
      }
    }
  }

  //Selects all items that are dated for this week and displays balance gains and loss in the chart
  updateWeekChart() {
    let day = new Date(),
    ...
    for (let iDay = firstWeekDay; iDay <= today; iDay++) {
      ...
      for (let i = 0; i < this.widgetList.length; i++) {
        if (this.widgetList[i].date === year + "-" + month + "-" + iDay ||
          this.widgetList[i].date === year + "-0" + month + "-" + iDay) {
  ...
  //Groups items that are in the same category (from this month) and displays them
  updateCategoryChart() {
    const categories = [
      "Freizeit",
      "Haushalt",
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      for (let j = 0; j < this.widgetList.length; j++) {
        ...
        if ((currentMonth === month || "0" + currentMonth === month) && this
          .widgetList[j].category === categories[i]) {
  ...
  //Checks how much has been added to the "Sparen" category this month and calculates progress percentage to goal
  updateSavingsChart() {
    let date = new Date(),
      currentMonth = date.getMonth() + 1,
      alreadySaved = 0,
